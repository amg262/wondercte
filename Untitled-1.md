...
- **Better Auth** for OAuth (Google/Facebook) authenticationI want to add the feature that compares a leaderboard's wonderlic result with an NFL players score. I also need to generate some seed data for the leaderboard, so when I test out the application features I can validate they are working now.
